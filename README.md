...
- **Mute bars** (also called "silent bars" or "drop-out" in other apps) — set a number of play bars followed by a number of mute bars; the cycle repeats indefinitely to train internal pulse. When Play bars = 0 the feature is disabled (default). Optional **Visual** checkbox silences the green beat-button flash during mute bars for a full blackout challenge
...
- **Import history** — load a previous Excel log to restore full history on a new device; replaces current history entirely with file contents (with confirmation prompt)
...
3. Optionally set Play bars / Mute bars for internal-pulse training
4. Press **Start** — first random root note appears, metronome begins, screen stays on
5. Play in tempo, press **Next Note** (or **Spacebar** on desktop)
6. Repeat until **12/12** — the last **Next Note** becomes **Save**
7. Session is scored automatically and Excel file downloads
